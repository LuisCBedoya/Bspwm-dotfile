...
then 
    ...
    do
        ...
        then
            notify-send --urgency=critical "$bat_percent% : Bateria Baja, Conecta el cargador por favor!"
            launched=$((launched+1))
        elif [[ $bat_percent -eq $FULL_BAT && "$bs" = "Charging" ]]
        then
	    notify-send --urgency=critical "$bat_percent% : Bateria llena, Desconecta el cargador por favor"
            launched=0
        fi
        sleep $POLL_INTERVAL
    done
